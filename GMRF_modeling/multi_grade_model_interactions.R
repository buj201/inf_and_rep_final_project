...
if (test_flag){
  ...
  warmup = 100
  num_chains = 1
  file_string = "./stan_models/multivar_model_interactions_stan_fit_test.RData"
} else {
  num_iter = 1500
  warmup = 500
  num_chains = 4
  file_string = "./stan_models/multivar_model_interactions_stan_fit.RData"
}
...
for (i in 1:K){
  ...
  uncorrelated_params = lm(formula = vec_counts ~ vec_grade + vec_time + vec_grade*vec_time,
                           data = data_list)
  ...
  init_beta_gt[i] = uncorrelated_params[4]
}
...
pars = c("alpha_0","alpha_g","alpha_t",'alpha_gt',
         "beta_0","beta_g","beta_t",'beta_gt',
         'tau_sq_0','tau_sq_g','tau_sq_t','tau_sq_gt',
         "rho_0","rho_g","rho_t",'rho_gt',
         'v_sq', 'mu')

multi_grade_interactions_stan_model = stan(file="multi_grade_model_interactions.stan",
                  data = data,
                  pars = pars,
                  warmup = warmup,
                  iter = num_iter,
...
#save fit stan model:
save(multi_grade_interactions_stan_model, file = file_string)
...
coeffs_for_model = c('beta_0','beta_t',
                     ...
                     'alpha_0','alpha_t',
                     'alpha_g','alpha_gt')
post_mean = get_posterior_mean(multi_grade_interactions_stan_model, pars=coeffs_for_model)[,5] #all chains
...
for (i in 1:K){
  ...
  baseline_model_K = lm(formula = vec_counts ~ vec_grade + vec_time + vec_grade*vec_time,
...
write.table(results, './stan_models/multi_grade_model_interaction_scores.csv',sep=',')
