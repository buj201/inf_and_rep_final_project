...
diagonal_change_model_corr = stan(file="exponential_cohort_model.stan",
                              data = corr_data,
                              pars = corr_pars,
                              warmup = warmup,
                              iter = num_iter,
...
diagonal_change_model_no_corr = stan(file="exponential_cohort_model_no_spatial_corr.stan",
                                  data = no_corr_data,
                                  pars = no_corr_pars,
                                  warmup = warmup,
                                  iter = num_iter,
...
save(diagonal_change_model_corr, file = "./stan_models/exponential_change_corr_stan_fit.RData")
save(diagonal_change_model_no_corr, file = "./stan_models/exponential_change_no_corr_stan_fit.RData")
        ...
        while ((r + num_ahead) <= nrow(data_matrix) & (c + num_ahead) <= ncol(data_matrix)){
          sum_SE_by_given_predict[r, r + num_ahead] = sum_SE_by_given_predict[r, r + num_ahead] +
            (data_matrix[r+num_ahead, c+num_ahead] -
               (data_matrix[r,c]*(1+rate)^num_ahead))^2
...
#save fit stan models and error matrices:
write.table(corr_RMSE_errors, './stan_models/exponential_change_corr_RMSE_error.csv',sep=',')
write.table(no_corr_RMSE_errors, './stan_models/exponential_change_no_corr_RMSE_error.csv',sep=',')
