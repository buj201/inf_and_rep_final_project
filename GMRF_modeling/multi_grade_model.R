...
if (test_flag){
  ...
  warmup = 100
  num_chains = 1
  file_string = "./stan_models/multivar_model_stan_fit_test.RData"
} else {
  num_iter = 1500
  warmup = 500
  num_chains = 4
  file_string = "./stan_models/multivar_model_stan_fit.RData"
}
...
for (i in 1:K){
  ...
  uncorrelated_params = lm(formula = vec_counts ~ vec_grade + vec_time,
                           data = data_list)
...
pars = c("alpha_0","alpha_g","alpha_t",
         "beta_0","beta_g","beta_t",
         'tau_sq_0','tau_sq_g','tau_sq_t',
         "rho_0","rho_g","rho_t",
         'v_sq', 'mu')

multi_grade_stan_model = stan(file="multi_grade_lin_model.stan",
                  data = data,
                  pars = pars,
                  warmup = warmup,
                  iter = num_iter,
...
#save fit stan model:
save(multi_grade_stan_model, file = file_string)
...
coeffs_for_model = c('beta_0','beta_t',
                     ...
                     'alpha_0','alpha_t',
                     'alpha_g')
post_mean = get_posterior_mean(multi_grade_stan_model, pars=coeffs_for_model)[,5]#All chains
...
for (i in 1:K){
  ...
  baseline_model_K = lm(formula = vec_counts ~ vec_grade + vec_time, data = train_data_list)
...
write.table(results, './stan_models/multi_grade_model_scores.csv',sep=',')
